...
function validateAuto(el, alert){
	...
	$('form[id="'+idForm+'"] input[type="text"]:enabled, form[id="'+idForm+'"] select:enabled, form[id="'+idForm+'"] textarea:enabled').each(function () {
            var input = $(this);
...
function validateForId(forId, alert){
    ...
    if($('#'+idForm)){
        $('#'+idForm+' input[type="text"]:enabled, #'+idForm+' select:enabled, #'+idForm+' textarea:enabled').each(function(){
            var input = $(this);
            if (!checkForErrors(input)) {
                result = false;
            }
...
function validateAll(alert){
	...
	$('input[type="text"]:enabled, select:enabled, textarea:enabled').each(function(){
		var input = $(this);
		if (!checkForErrors(input)) {
			result = false;
		}
...
function jQAlertError(resultado, alerta, idForm){
	...
	if (!resultado && (alerta==true)) {
		var numErrs = $(idCheck+'.errorField').length
